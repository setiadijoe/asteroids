...
fn main() {
    ...
    // Create all meteoroids
    let meteoroids = meteoroids();
    for (i, meteoroid) in meteoroids.into_iter().enumerate() {
        let sprite = game.add_sprite(format!("{ID_METEOROID}{}", i), meteoroid);
...
fn game_logic(engine: &mut Engine, game_state: &mut GameState) {
    if game_state.stop {
        ...
        if engine.keyboard_state.pressed(KeyCode::R) {
            engine.texts.remove(ID_GAMEOVER);
            ...
            generate_meteoroids(engine, game_state);
    ...
    if engine.keyboard_state.pressed_any(&[KeyCode::Right, KeyCode::F]) {
    ...
    if engine.keyboard_state.pressed_any(&[KeyCode::Up, KeyCode::E]) && game_state.thrust_timer.finished() {
        give_thrust = true;
        game_state.thrust_timer.reset();
    }
    ...
    for sprite in engine.sprites.values_mut() {
        // bounds check
        if sprite.translation.y > 360.0 || sprite.translation.y < -360.0
            || sprite.translation.x > 800.0 || sprite.translation.x < -800.0 {
                ...
                if sprite.label.starts_with(ID_SHOT) {
                    game_state.sprites_to_delete.push(sprite.label.clone());
                    continue;
                }
            }
        if sprite.label.starts_with(ID_SHOT) {
            sprite.translation.x += SHOT_SPEED * engine.delta_f32 * (sprite.rotation as f64).cos() as f32;
            sprite.translation.y += SHOT_SPEED * engine.delta_f32 * (sprite.rotation as f64).sin() as f32;
        }

        if sprite.label.starts_with(ID_METEOROID) {
            ...
            sprite.translation.y += speed * engine.delta_f32 * (sprite.rotation as f64).sin() as f32;

            // bounds check, out on left side -> new random position
            if sprite.translation.x < min_x || sprite.translation.x > max_x 
                || sprite.translation.y < min_y || sprite.translation.y > max_y {
    ...
    if shoot {
        ...
        let sprite = engine.add_sprite(
            format!("{ID_SHOT}{}", game_state.shot_counter),
    ...
    for event in engine.collision_events.drain(..) {
        ...
        if event.pair.one_starts_with(ID_PLAYER) {
            if event.pair.either_starts_with(ID_SHOT) {
                continue;
            }  
            if event.pair.either_starts_with(ID_METEOROID) {
                engine.audio_manager.play_sfx(SfxPreset::Impact3, 0.5);
                game_over = true;
                break;
            }
        } 
        if event.pair.one_starts_with(ID_SHOT) && event.pair.one_starts_with(ID_METEOROID){
    ...
    if game_over || game_state.meteoroids.is_empty() {
        ...
        if game_over {
            let game_over_text = engine.add_text(ID_GAMEOVER, "You Lost!");
            game_over_text.font_size = 128.0;
            engine.audio_manager.play_sfx(SfxPreset::Jingle3, 0.5);
        } else if game_state.meteoroids.is_empty() {
            let you_won_text = engine.add_text(ID_GAMEOVER, "You Won!");
...
fn generate_meteoroids(engine: &mut Engine, game_state: &mut GameState) {
    let meteoroids = meteoroids();
    for (i, meteoroid) in meteoroids.into_iter().enumerate() {
        let sprite = engine.add_sprite(format!("{ID_METEOROID}{}", i), meteoroid);
        reset_meteoroid(sprite, game_state);
        // Save the name of the Meteoroid so that we can
        // keep track of when all of them are gone.
        game_state.meteoroids.push(sprite.label.clone());
